...
Looks for a `.env` file in (priority order):
  ...
  2. <skill_root>/.env  (i.e. %USERPROFILE%/.codex/skills/media-tools/.env)
  3. ~/.config/media-tools/.env (or %APPDATA%\media-tools\.env on Windows)

Existing os.environ values are NEVER overwritten â€” env wins over file.
...
def _skill_root() -> Path:
    # this file: %USERPROFILE%/.codex/skills/media-tools/scripts/lib/dotenv_loader.py

...
- **Fallback CLI mode:** `scripts/image_gen.py` CLI. Use when the user explicitly asks for the CLI/API/model path, or after the user explicitly confirms a true model-native transparency fallback with `gpt-image-1.5`. Requires a local OpenAI auth credential (`OPENAI_API_KEY`, `OPENAI_ACCESS_TOKEN`, `OPENAI_OAUTH_ACCESS_TOKEN`, `CODEX_OPENAI_ACCESS_TOKEN`, or `OPENAI_AUTH_JSON_FILE` pointing to JSON with `accessToken`).
...
- If the built-in tool fails or is unavailable, tell the user the CLI fallback exists and that it requires a local OpenAI auth credential. Proceed only if the user explicitly asks for that fallback.
...
```text
This likely needs true native transparency. The default built-in path uses a chroma-key background plus local removal, but true transparency requires the CLI fallback with gpt-image-1.5 because gpt-image-2 does not support background=transparent. It also requires a local OpenAI auth credential. Should I proceed with that CLI fallback?
```
...
### Environment
- A local OpenAI auth credential must be set for live API calls.
- Supported credentials, in priority order: `OPENAI_API_KEY`, `OPENAI_ACCESS_TOKEN`, `OPENAI_OAUTH_ACCESS_TOKEN`, `CODEX_OPENAI_ACCESS_TOKEN`, `OPENAI_AUTH_JSON_FILE`, `CODEX_OPENAI_AUTH_JSON_FILE`.
- `OPENAI_AUTH_JSON_FILE` / `CODEX_OPENAI_AUTH_JSON_FILE` must point to a local JSON file containing an `accessToken` string.
- Do not ask the user for credentials when using the built-in `image_gen` tool.
- Never ask the user to paste a full key or token in chat. Ask them to set it locally and confirm when ready.

If auth is missing, give the user these steps:
1. Create an API key in the OpenAI platform UI or use a local OAuth access token source.
2. Set `OPENAI_API_KEY` for an API key, or `OPENAI_ACCESS_TOKEN` for an OAuth bearer token.
3. For OAuth JSON, save it in a local ignored file and set `OPENAI_AUTH_JSON_FILE` to that path.

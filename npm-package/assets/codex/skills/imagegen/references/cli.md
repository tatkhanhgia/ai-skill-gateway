...
Real API calls require **network access** + a local OpenAI auth credential. `--dry-run` does not.
...
Generate (requires local OpenAI auth + network):
...
Supported live-call credentials, in priority order:
- `OPENAI_API_KEY`
- `OPENAI_ACCESS_TOKEN`
...
`OPENAI_AUTH_JSON_FILE` / `CODEX_OPENAI_AUTH_JSON_FILE` must point to a local JSON file containing an `accessToken` string. Do not paste keys or tokens into chat, source files, command history, or committed project files.

...
- Generate images (host model may not generate images directly) — pick from OpenRouter, Gemini, OpenAI gpt-image-2, MiniMax, DashScope, BytePlus, **or your existing ChatGPT Plus / Codex subscription** (no extra API key needed)
...
```powershell
.opencode\skills\.venv\Scripts\python.exe -m pip install -r .opencode\skills\media-tools\requirements.txt
```
...
```powershell
copy .opencode\skills\media-tools\.env.example .opencode\skills\media-tools\.env
notepad .opencode\skills\media-tools\.env
```

The loader searches in this priority order:

1. `$MEDIA_TOOLS_ENV` (override path)
2. `.opencode/skills/media-tools/.env` ← **default, recommended**
...
```powershell
.opencode\skills\.venv\Scripts\python.exe .opencode\skills\media-tools\scripts\chatgpt_oauth_login.py
...
```powershell
.opencode\skills\.venv\Scripts\python.exe .opencode\skills\media-tools\scripts\create_image.py `
...
```powershell
.opencode\skills\.venv\Scripts\python.exe .opencode\skills\media-tools\scripts\read_image.py `

...
2. **`~/.opencode/agents/media-designer.md`** — agent profile that *uses* the search tool to turn a user brief into a paste-ready GPT-Image-2 prompt.
...
```
1. Read ~/.opencode/agents/media-designer.md
...
#   Windows : .opencode\skills\.venv\Scripts\python.exe
#   POSIX   : .opencode/skills/.venv/bin/python3
# Examples below use the Windows path; swap on POSIX.

# Free-text search (this is what the agent calls)
.opencode\skills\.venv\Scripts\python.exe .opencode\skills\gpt-image-2-pro-max\scripts\search.py "luxury shoe ecommerce ad cream pastel" -n 5

# Narrow by shape when the brief is specific about format
.opencode\skills\.venv\Scripts\python.exe .opencode\skills\gpt-image-2-pro-max\scripts\search.py "perfume bottle" --shape ecommerce -n 3

# Persist top hits as a markdown reference deck (with embedded images)
.opencode\skills\.venv\Scripts\python.exe .opencode\skills\gpt-image-2-pro-max\scripts\search.py "neon ui" --persist plans\neon-refs.md
```
...
`.opencode/agents/media-designer.md` defines the canonical workflow. Headline contents:
...
| Step 2 — Search | Synthesise tokens, run `.opencode/skills/.venv/Scripts/python.exe .opencode/skills/gpt-image-2-pro-max/scripts/search.py "<tokens>" -n 5` |

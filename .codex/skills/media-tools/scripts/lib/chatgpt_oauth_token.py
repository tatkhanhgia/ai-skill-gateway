...
def _parse_iso_datetime(value: str) -> float:
    """Parse ISO timestamps such as 2026-05-16T03:14:21.941Z."""
    if not value:
        return 0.0
    try:
...
def _load_from_auth_json_env() -> OAuthSession | None:
    """Load OAuth session from OPENAI_AUTH_JSON_FILE / CODEX_OPENAI_AUTH_JSON_FILE.

    Supports provider records shaped like:
      { "accessToken": "...", "refreshToken": "...", "expiresAt": "..." }
    This keeps credentials in a local file instead of command-line args.
    """
    raw_path = (
        os.environ.get("OPENAI_AUTH_JSON_FILE")
        or os.environ.get("CODEX_OPENAI_AUTH_JSON_FILE")
        or ""
    ).strip()
    if not raw_path:
        return None
    ...
    expires = raw.get("expiresAt") or raw.get("expires_at") or ""
    if not access:
        return None
    provider_data = raw.get("providerSpecificData") or {}
    return OAuthSession(
        access_token=str(access),
        refresh_token=str(refresh),
        expires_at=_parse_iso_datetime(str(expires)) or _decode_jwt_exp(str(access)),
        api_base=DEFAULT_API_BASE,
        account_id=str(provider_data.get("chatgptAccountId", "") or ""),
        plan_type=str(provider_data.get("chatgptPlanType", "") or ""),
        scope="",
    )
...
def load_session() -> OAuthSession | None:
    ...
    return _load_from_auth_json_env() or _load_from_codex() or _load_from_media_tools()

...
AUTH_JSON_FILE_ENV_NAMES = (
    "OPENAI_AUTH_JSON_FILE",
    "CODEX_OPENAI_AUTH_JSON_FILE",
)
...
def _extract_access_token(raw: str, source: str) -> str:
    ...
    try:
        ...
        _die(f"{source} looks like JSON but could not be parsed: {exc}")
    token = data.get("accessToken")
    if not isinstance(token, str) or not token.strip():
        _die(f"{source} JSON must contain a non-empty accessToken string.")
    return token.lstrip("\ufeff").strip()

def _read_auth_json_file(path: str, source: str) -> str:
    auth_path = Path(path)
    if not auth_path.exists():
        _die(f"{source} points to a missing file: {auth_path}")
    return _extract_access_token(auth_path.read_text(encoding="utf-8"), source)

def _get_auth_token() -> Tuple[Optional[str], Optional[str]]:
    for name in AUTH_ENV_NAMES:
        raw = os.getenv(name)
        if raw:
            return _extract_access_token(raw, name), name

    for name in AUTH_JSON_FILE_ENV_NAMES:
        raw_path = os.getenv(name)
        if raw_path:
            return _read_auth_json_file(raw_path, name), name
...
def _ensure_api_key(dry_run: bool) -> None:
    ...
    if token:
        print(f"OpenAI auth credential is set ({source}).", file=sys.stderr)
        return
    if dry_run:
        _warn("OpenAI auth credential is not set; dry-run only.")
        return
    _die(
        "OpenAI auth credential is not set. Export OPENAI_API_KEY, "
        "OPENAI_ACCESS_TOKEN, OPENAI_OAUTH_ACCESS_TOKEN, CODEX_OPENAI_ACCESS_TOKEN, "
        "or set OPENAI_AUTH_JSON_FILE to a JSON file containing accessToken."
    )
